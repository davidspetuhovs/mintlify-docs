...
<Tabs>
  <Tab title="Users">
    ...
    **Range filters:**
    ...
    - Date range — filter users by when they were first seen

    The table is paginated. You can sort by any column in ascending or descending order.
  ...
  <Tab title="Organizations">
    ...
    **Range filters:**
    ...
    - MRR (min/max)
    - Date range — filter organizations by when they were first seen
...
<Info>
  Fields not provided appear as empty in the table and are not available as filters. Richer metadata means richer filtering and sorting. See the [SDK documentation](/sdk/recording-messages) for the full metadata schema.
</Info>
...
Sort organizations by user count or conversation count to find your most and least engaged accounts. Low adoption in a high-MRR account may indicate onboarding issues or a need for enablement outreach.

### Account health

For any user, see conversation volume and sentiment trend at a glance. At the organization level, sort by sentiment ascending and filter by MRR to find at-risk high-value accounts before they escalate.
