...
This flow allows users to **email/password login** from the Login Page page.
...
Verify that the "Email/password login" completed successfully and the expected state change occurred.
...
```
http://localhost:3000 → /auth/login → [Email/password login]
```
