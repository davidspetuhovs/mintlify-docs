...
This flow allows users to **change date range** from the Chatbot Dashboard page.
...
### Step 2: Interact with button: date range selector

Use the **button: date range selector** element to progress through the flow.

{{screenshot_2}}

### Step 3: Verify outcome

Verify that the "Change date range" completed successfully and the expected state change occurred.
...
```
http://localhost:3000 → /platform/[chatbotId] → [Change date range]
```
